...
止损、放量验证、放量成功/失败和衰退主因的量化阈值见 [[Google_Meta_Campaign生命周期投放优化决策指南]]；Meta 侧执行要点：先看素材和审核恢复（素材疲劳先换广告，72h 内换过广告则继续观察），再动预算；进入放量验证前确认 live creative >= 2 且 top creative 花费占比 <= 75%，单素材过度集中时短期 ROI 好看也不放量。
...
测试结构（1-1-N / 1-N-1 / 1-N-N）的选择原则、测试+放量双结构范式和低预算批量打法的演进分析，见 [[Meta测试结构与投放策略演进]]。
